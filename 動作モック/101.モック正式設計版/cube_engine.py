...
class CubeEngine:
    ...
    def _extract_keyphrases(self, text: str) -> List[Dict]:
        ...
        prompt = f"""【あなたへのオーダー】
以下のテキストを解析して、代表的な「主語」と「述語」、それから重要な名詞や概念を表すキーフレーズを抽出してください。
抽出した各キーフレーズに対して、0.0〜1.0の範囲で信頼度を付与してください。
指示が多いですが頑張ってください。

- また、その会話が何のジャンルについての会話なのかを推測して、role:genre, role:topic, role:problem/theme を付与してください。
- ジャンルが特定できない場合は、role:genre, role:topic, role:problem/theme は無しとしてください。

例題1：『今日は友達と久しぶりにボールを蹴りに行った。PK合戦をやって負けた。』この文章なら
{{"phrase": "スポーツ", "role": "genre", "confidence": 0.8}},
{{"phrase": "サッカー", "role": "topic", "confidence": 0.6}},
{{"phrase": "試合をやって負けた", "role": "problem/theme", "confidence": 0.6}}
の様になります。

例題2：『メディアは依然として影響力が強いが、偏向報道の問題が玉に瑕だ』➔ 
{{"phrase": "メディア", "role": "genre", "confidence": 0.9}},
{{"phrase": "メディアの影響力", "role": "topic", "confidence": 0.8}},
{{"phrase": "偏向報道の問題", "role": "problem/theme", "confidence": 0.6}}
となります。

例題3：[うんうん, こんにちは, そうですね, なるほど] ➔ role:genre, role:topic, role:problem/theme は無しになります。
...
- "topic": 会話のトピック（例：サッカー、AI、気候変動など）
- "problem/theme": 何の問題を扱っているかやテーマ（例：偏向報道、環境問題など）
- "subject": 主語（何が、誰が）
