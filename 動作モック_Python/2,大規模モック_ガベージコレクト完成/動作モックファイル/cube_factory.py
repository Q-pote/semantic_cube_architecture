...
def make_cube(text: str, role: str, turn: int, ref_count: float, llm_model_name: str) -> SemanticCube:
    vec = embed_text(text)
    ...
    print(f"🧊 [Cube Factory] ID:{cube.cube_id} / Role:{cube.role:7s} / Alpha:{cube.trust.alpha:.2f} / MiniCubes:{len(cube.mini_cubes)}")
    return cube
