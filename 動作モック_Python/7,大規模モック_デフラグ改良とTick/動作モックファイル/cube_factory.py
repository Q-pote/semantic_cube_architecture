...
def embed_text(text: str) -> np.ndarray:
    ...
    emb_model = os.environ.get("GEMINI_EMBEDDING_MODEL", "gemini-embedding-2")
    try:
...
以下のテキストから文章の象徴的なキーワードを１～８個程度抽出してください。
...
def make_cube(text: str, role: str, turn: int, ref_count: float, llm_model_name: str, parent_id: str = None, origin_id: str = None, response: str = None) -> SemanticCube:
    vec = embed_text(text)
    ...
    # ✨ 修正: 実時刻を取得して created_at_time に注入するにゃ

    trust = TrustStruct(
        grid_index=grid, role=role, orientation=ori, 
        ...
        # ✨ 追加: 生成ターンの記録とベースアルファの初期化
        last_updated_turn=turn,
        alpha_base=1.0
    )
    # 明示的にクリップ（冗長だが安全）
    trust.alpha_base = max(0.0, min(1.0, trust.alpha_base))
    ...
    print(f"🧊 [Cube Factory] ID:{cube.cube_id} / Role:{cube.role:7s} / Alpha:{cube.trust.alpha_base:.2f} / Parent:{parent_id} / Origin:{cube.origin_id} / Time:{cube.trust.created_at_time}")
    return cube
