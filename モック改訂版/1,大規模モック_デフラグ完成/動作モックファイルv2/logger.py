...
def export_cubes_to_json(cubes: list[SemanticCube], query_cube: SemanticCube = None) -> str:
    """
    キューブ群の空間座標データとMiniCube情報をJSON文字列として出力する。
    """
    export_data = {"cubes": []}
    ...
    for c in cubes:
        cube_data = {
            ...
            "orientation": {
                "azimuth": round(c.trust.orientation.azimuth, 4),
                "elevation": round(c.trust.orientation.elevation, 4),
                "strength": round(c.trust.orientation.strength, 4)
            },
    ...
    if query_cube:
        export_data["query_cube"] = {
            ...
            "orientation": {
                "azimuth": round(query_cube.trust.orientation.azimuth, 4),
                "elevation": round(query_cube.trust.orientation.elevation, 4),
                "strength": round(query_cube.trust.orientation.strength, 4)
            },
